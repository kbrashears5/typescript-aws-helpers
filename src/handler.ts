...
import {
  APIGatewayHelper,
  IAPIGatewayHelper,
} from 'typescript-aws-apigateway-helper';
import {
  CloudWatchHelper,
  ICloudWatchHelper,
} from 'typescript-aws-cloudwatch-helper';
...
export class Handler implements IHandler {
  ...
  public async Execute<T>(
    ...
    if (this.ObjectOperations.IsNullOrEmpty(context)) {
      throw new Error(`[${action}]-Must supply context`);
    }
  ...
  public async OrchestrateSQS(
    ...
    if (this.ObjectOperations.IsNullOrWhitespace(queueUrl)) {
      throw new Error(`[${action}]-Must supply queueUrl`);
    }
